...
import { getAllRecords, MindRecord, deleteRecord, updateRecord } from "../../services/mind.service";
...
export default function RecordPage() {
            ...
            .then((res) => {
                ...
                if (Array.isArray(data)) {
                    ...
                    if (Array.isArray(data.records)) {
                        console.log('✅ 找到 data.records,长度:', data.records.length);
                        recordsList = data.records;
                    } else if (Array.isArray(data.data)) {
                        console.log('✅ 找到 data.data,长度:', data.data.length);
                        recordsList = data.data;
                    } else if (Array.isArray(data.list)) {
                        console.log('✅ 找到 data.list,长度:', data.list.length);
                        recordsList = data.list;
                    } else if (Array.isArray(data.result)) {
                        console.log('✅ 找到 data.result,长度:', data.result.length);
                        recordsList = data.result;
                    } else if (Array.isArray(data.items)) {
                        console.log('✅ 找到 data.items,长度:', data.items.length);
                        recordsList = data.items;
                    } else if (data.code === 200 && Array.isArray(data.data)) {
                        console.log('✅ 找到 data.code=200 且 data.data,长度:', data.data.length);
                        recordsList = data.data;
                    } else if (data.status === 200 && Array.isArray(data.data)) {
                        console.log('✅ 找到 data.status=200 且 data.data,长度:', data.data.length);
                        recordsList = data.data;
                    } else {
                        // 如果都不匹配，尝试找到第一个数组类型的值
                        console.log('⚠️ 未找到标准字段，尝试查找数组类型的值...');
    ...
    const getRecordTime = (record: any): string | undefined => {
        return record.CreatedAt || record.created_at || record.createdAt || record.create_time || record.createTime || record.time || record.timestamp;
    };
                                ...
                                >
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                            <div style={{ fontSize: 14, fontWeight: 600, color: "#2b8a66" }}>
                                    ...
                                    {editingId === getRecordId(r) ? (
                                        ...
                                        <div style={{ fontSize: 14, color: "#333", whiteSpace: "pre-wrap", lineHeight: 1.6, marginBottom: 8 }}>
                                            {r.content}
                                        </div>
                                    )}
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                        <div style={{ fontSize: 11, color: "#999", display: "flex", alignItems: "center", gap: 4 }}>
                                            <span>📅</span>
                                            <span>创建时间: {renderTime(getRecordTime(r))}</span>
                                        </div>
                                        {editingId !== getRecordId(r) && (
                                            <div style={{ display: "flex", gap: 8 }}>
                                                <Button
